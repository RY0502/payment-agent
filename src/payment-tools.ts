        ...
        try {
          ...
          const qrImageUrl = await page.evaluate(() => {
            ...
            for (const svg of allSvgs) {
              ...
              if (hasQrContext && looksLikeQr) {
                ...
                // Inline SVG to data URL conversion with proper UTF-8 encoding to prevent data corruption
                try {
                  const serializer = new XMLSerializer();
                  const svgString = serializer.serializeToString(svg);
                  console.log(`📊 [Browser] Original SVG size: ${svgString.length} characters`);
                  
                  // Convert to UTF-8 bytes then to base64 (preserves Unicode, prevents corruption)
                  // This is safer than btoa() which only handles ASCII
          ...
          if (notificationUrl && supabaseAnonKey) {
            try {
              ...
              // Debug: Log data URL format details
              console.log('🔍 Data URL prefix (first 100 chars):', qrImageUrl.substring(0, 100));
              console.log('🔍 Data URL starts with "data:"?', qrImageUrl.startsWith('data:'));
              console.log('🔍 Data URL includes ";base64,"?', qrImageUrl.includes(';base64,'));
